...
while True:
    ...
    for letter in mira_data:
        ...
        data = data
        new_points = abs_coords.convert(data.T)
        x = new_points[0, 0]-80
        y = new_points[1, 0]
        z = new_points[2, 0]
        arm.set_position(x=x, y=y, z=z+2, roll=180, pitch=0, yaw=0, speed=100, is_radian=0, wait=True,radius = None, relative = False)
        ...
        for i in range(0, new_points.shape[1]+1):
            ...
            x = new_points[0, idx]-80
            y = new_points[1, idx]
            z = new_points[2, idx]
            arm.set_position_aa([x, y, z, 180, 0, 0], speed=100, is_radian=0, wait=True,radius = None, relative = False,mvacc=2000)
