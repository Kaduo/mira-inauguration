...
def big_dawg(image, sigma, p, dilation_size):
    dog_response = ski.color.rgb2gray(difference_of_gaussians(image, sigma, high_sigma=1.7*sigma, p=p))
    thresh_otsu = dog_response > threshold_otsu(dog_response)
    edge_image_otsu = 1 - (thresh_otsu ^ dilation(thresh_otsu, footprint=np.ones((dilation_size, dilation_size))))
    thresh_mean = dog_response > threshold_mean(dog_response)
    edge_image_mean = 1 - (thresh_mean ^ dilation(thresh_mean, footprint=np.ones((dilation_size, dilation_size))))
...
dog_response, thresh_otsu, thresh_mean, edge_image_otsu, edge_image_mean = big_dawg(image, sigma=init_sigma, p=init_p, dilation_size=init_dilation_size)
...
def update(val):
    dog_response, thresh_otsu, thresh_mean, edge_image_otsu, edge_image_mean = big_dawg(image, sigma=sigma_slider.val, p=p_slider.val, dilation_size=int(dilate_slider.val))
